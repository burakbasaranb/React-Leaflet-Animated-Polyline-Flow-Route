...
const MyMap = () => {
  ...
  useEffect(() => {
    ...
    // Car Animation
    // Istanbul_Anatolia
    var pathPatternCar = L.polylineDecorator(
      [Istanbul_Anatolia],
      {
          patterns: [
            { offset: '0%', repeat: '0', symbol: L.Symbol.marker({rotate: true, markerOptions: {
                icon: L.icon({
                    //iconUrl: 'http://bbecquet.github.io/Leaflet.PolylineDecorator/example/icon_plane.png',
                    iconUrl: '../../../icon_car.png',
                    iconAnchor: [16, 16]
                })
            }})}
          ]
    ...
    var offsetPercentageCar = 0;
  
    setInterval(function () {
        // Update the offset
        map.removeLayer(pathPatternCar);
        pathPatternCar = L.polylineDecorator(
          [Istanbul_Anatolia],
          {
              patterns: [
                { offset: offsetPercentageCar+'%', repeat: '0', symbol: L.Symbol.marker({rotate: true, markerOptions: {
                    icon: L.icon({
                        iconUrl: '../../../icon_car.png',
                        iconAnchor: [16, 16]
                    })
        ...
        // Increment the offset for the next animation frame
        offsetPercentageCar += 0.5;
  
        // Reset the offset to the beginning when it reaches 100%
        if (offsetPercentageCar > 100) {
          offsetPercentageCar = 0;
        }
    }, 200);
    //---------------------------------------------------------
    // Plane Animation
    // Istanbul_Anatolia
    var pathPatternPlane = L.polylineDecorator(
      [Istanbul_Anatolia],
      {
          patterns: [
            { offset: '0%', repeat: '0', symbol: L.Symbol.marker({rotate: true, markerOptions: {
    ...
    var offsetPercentagePlane = 0;
  
    setInterval(function () {
        // Update the offset
        map.removeLayer(pathPatternPlane);
        pathPatternPlane = L.polylineDecorator(
          [Istanbul_Euro],
          {
              patterns: [
                { offset: offsetPercentagePlane+'%', repeat: '0', symbol: L.Symbol.marker({rotate: true, markerOptions: {
                    icon: L.icon({
                        //iconUrl: 'http://bbecquet.github.io/Leaflet.PolylineDecorator/example/icon_plane.png',
                        iconUrl: '../../../icon_plane.png',
                        iconAnchor: [16, 16]
        ...
        // Increment the offset for the next animation frame
        offsetPercentagePlane += 0.5;
  
        // Reset the offset to the beginning when it reaches 100%
        if (offsetPercentagePlane > 100) {
          offsetPercentagePlane = 0;
        }
    }, 200);
    //---------------------------------------------------------
  }, [CusromCRS]);
  ...
  //console.log(mapRef.current.props.crs)
    ...
    return (
      ...
      <LayersControl>
        ...
        <Polyline smoothFactor={5} color="darkred" positions={Istanbul_Euro} weight="1" arrowheads={{ size: '5px' }}>
        <Popup>
          <code>{`<Polyline smoothFactor={5} color="darkred" positions={Istanbul_Euro} arrowheads={{ size: '5px' }}>`}</code>
        </Popup>
      </Polyline>

      <Polyline smoothFactor={5} color="blue" positions={Istanbul_Anatolia} weight="1" arrowheads={{ size: '5px' }}>
        <Popup>
